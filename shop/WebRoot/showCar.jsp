		...
		$(function(){
			$(".text").change(function(){
				...
				if(!isNaN(number) && parseInt(number)==number &&number>0){
					...
					//发送ajax请求，传输当前数量，返回的是总价格
					$.post("sorder_updateSorder.action",{number:number,'product.id':pid},function(result){
						//var price=$("table .price").html();
						$("#total").html(result);
						$("#priceinfo").html(result);
					},"text");
						var price=$(this).parent().next().html();
						price=price.substring(0,price.indexOf('*'));
						$(this).parent().next().html(price+'*'+number);
				}else{
				...
				<div class="totals">
						...
						<tbody>
							<tr>
								...
								<td class="align_right" style=""><strong><span
										class="price" id="priceinfo">${sessionScope.forder.total}</span>
								</strong>
								</td>
							</tr>
							<tr>
								<td width="60%" colspan="1" class="align_left">运费</td>
								<td class="align_right" style=""><span >￥0.00</span>
								</td>
							</tr>
							<tr>
					...
					<div class="action_buttonbar">
						<button type="button" title="" class="checkout fr"
							...
							<font><font><a href="${shop}/user/confirm.jsp" >提交订单</a></font>
							</font>
						</button>
